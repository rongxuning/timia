...
在用户允许「后台持续运行」后，于锁屏展示 Timia 实时信息：可选的健康同步状态 + 当日全天待办。
...
4. **展示（自上而下）**：
   - **健康区**：仅当已请求健康授权（`HealthPermissionManager.didRequest`）时显示「健康数据同步」+ 最后同步时间；未开启则整区隐藏。
   - **待办区**：当日全天任务（`end - start ≥ 23h` 或无 `startAt`）且状态为 `todo` / `doing`，每行「名称 + 时间」。
...
| `ScreenNotificationManager` | 启停/刷新 Live Activity；拉取当日日历全天待办；组装 ContentState |
| `ScreenNotificationPromptModifier` | 登录后一次性提示 UI |
| `TimiaWidget` 扩展 | 锁屏 / Dynamic Island Live Activity 视图 |
| Account「锁屏通知」开关 | 再次允许或关闭 |

数据源复用：`HealthSyncService.cachedLastSyncedAt()`、`GET /views/schedule/calendar?view=day`、与日历相同的全天判定。
