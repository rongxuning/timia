...
| A7 | 折叠偏好：记住「是否启用空闲折叠」到 `UserDefaults` | ✅ |
| A8 | 本日若含「当前时刻」，折叠时仍保留当前小时所在 **可见段**（或保证当前时间线可见） | ✅ |
| A9 | 周模式：**七天 busy 并集** 后算空闲并折叠，折叠条跨列对齐（W1） | ✅ 并集对齐 |
| A10 | **仅顶栏总开关**控制全部空闲展开/折叠；折叠段为视觉压缩条，**不提供**逐段展开按钮 | ✅ 2026-09-06 追加确认 |
| A11 | 拖拽进行中可临时强制展开全部（便于精确落点），松手后恢复顶栏开关状态 | ✅ |
...
- `collapsed` 段：矮条上可显示时间范围文案（如 `02:00 – 08:00`），**不可点按展开**。
...
- 单列 segments，空闲压缩条通栏。  
- **仅**时间轴最上方总开关：「折叠空闲」/「展开全部」，一次控制所有空闲段。

**周模式**

| 子方案 | 描述 | 推荐 |
|--------|------|------|
| **W1 按行对齐（推荐）** | **以「7 天并集 busy」** 算全局空闲——只有七天都空的时段才折叠 | 视觉整齐，列对齐 |
| **W2 每列独立折叠** | 同 y 可能一列折一列不折 | 网格错位难读 |

**推荐 W1 + 并集 busy。** 顶栏总开关同样控制整周。

### 6.4 折叠入口（已确认）

1. **唯一入口**：日/周时间轴**最上方**总开关 `折叠空闲` ↔ `展开全部`。  
2. **不做**逐空闲段展开按钮；压缩条仅作视觉分隔/示意。  
3. 状态只需：

```text
idleCollapseEnabled: Bool  // UserDefaults，默认 true
```

### 6.5 与新建 / 当前时间线

- 点空白（可见段）：`minutes(atY:)` → 新建仍 15 分钟 snap。  
- 点在压缩条：不新建（避免盲建）；也不单独展开该段。  
- `CurrentTimeLine`：用 `y(forMinutes: now)`；依赖 A8 保护保证今日当前时刻可见。

### 6.6 与拖拽联动

| 场景 | 行为 |
|------|------|
| 拖拽进行中 | **临时强制展开全部**（忽略顶栏折叠态），便于整点落点 |
| 松手后 | 恢复顶栏 `idleCollapseEnabled` 对应几何 |
| 折叠开关切换时正在拖 | **取消拖拽** |

动画：顶栏切换用 `withAnimation(.easeInOut(duration: 0.25))`。
...
2. **手测日模式**：稀疏日程自动折叠 → 顶栏「展开全部」→ 再「折叠空闲」；点空白新建落在真实时间；当前时间线可见；压缩条不可点展开。  
3. **手测周模式**：仅一天有早会时，深夜公共空闲可折；七天对齐；顶栏总开关控制整周。  
4. **拖拽**：日/周改期成功；同位置不请求；archived 不可拖；409 回滚；拖拽中临时全展开、松手后恢复折叠。  
