...
2. **本机远程打包**：`bash deploy/remote.sh`（默认 smart）
3. GitHub Actions SSH 只是辅助（国内轻量云常 `connection reset`）

细则：`docs/deploy/cloud.md`。编排：`docker-compose.prod.yml`。反代：`deploy/nginx.conf`。

## remote.sh（本机 → 轻量云）

默认对照**服务器当前 HEAD**，只处理有改动的服务：

```bash
bash deploy/remote.sh plan     # 列出清单
bash deploy/remote.sh          # 并行 build → 每服务单独 tar → 并行 upload/load
PACK_SERVICES=web bash deploy/remote.sh
```

- 未改动的服务不打包、不上传
- 产物：`deploy/dist/timia-<service>.tar.gz` + `timia-pack.manifest`
- SSH 不稳时：`PARALLEL_UPLOAD=0 bash deploy/remote.sh upload`

## 易错

- 服务：`web`、`core-service`、`file-service`、`mcp-server`、`db`、`nginx`
...
- 只重发 MCP：`DEPLOY_MODE=mcp-server bash deploy/local.sh` 或 `PACK_SERVICES=mcp-server bash deploy/remote.sh`
